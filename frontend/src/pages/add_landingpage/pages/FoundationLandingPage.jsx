...
const FloatingStickyBadge = ({ scrollToForm, onScholarshipClick }) => {
    const [studentCount, setStudentCount] = useState(1850);
...
export const FoundationLandingPage = () => {
    const [currentSlide, setCurrentSlide] = useState(0);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [showSuccess, setShowSuccess] = useState(false);
    const [centres, setCentres] = useState([]);
    const [isLeadCaptured, setIsLeadCaptured] = useState(() => {
        return localStorage.getItem('pathfinder_lead_captured_foundation') === 'true';
    });
    ...
    const allCourses = useMemo(() => {
        ...
        const filtered = dataArray.filter(c => {
            ...
            const isFoundation = name.includes('foundation') || target.includes('foundation') || cat.includes('foundation') ||
                desc.includes('foundation') || name.includes('7') || name.includes('8') || name.includes('9') || name.includes('10') ||
                classLevel.includes('7') || classLevel.includes('8') || classLevel.includes('9') || classLevel.includes('10');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data && (response.data.success || response.status === 201)) {
                setShowSuccess(true);
                localStorage.setItem('pathfinder_lead_captured_foundation', 'true');
    ...
    return (
        <div className="min-h-screen bg-gray-50 text-gray-900 selection:bg-orange-500 selection:text-white font-sans overflow-x-hidden">
            ...
            <div className="2xl:max-w-7xl mx-auto bg-white shadow-2xl relative">
                        ...
                        {isLeadCaptured ? (
                            <div className="py-20 text-center">
                                ...
                                <p className="text-xl md:text-2xl text-gray-300 font-medium max-w-2xl mx-auto leading-relaxed">
                                    Our expert academic counsellors will contact you soon to guide you through your Foundation Learning journey.
                                </p>
                            </div>
                        ) : (
                            <div className="w-full">
                                <div className="max-w-5xl mx-auto">
                                    <h2 className="text-3xl md:text-5xl font-bold mb-4 text-center leading-tight">
                                        Pathfinder <span className="text-[#FF9F00]">Foundation Program</span>
                                    </h2>
                                    <p className="text-center text-gray-300 text-sm md:text-base max-w-2xl mx-auto mb-10">
                                        Build a solid conceptual base for Olympiads, NTSE, Board Exams, and future Competitive Exams for Class 7, 8, 9 & 10.
                                    </p>
                                </div>
                ...
                {/* Why Choose Pathfinder Foundation Section */}
                    ...
                    <div className="max-w-7xl mx-auto px-6 relative z-10">
                        <div className="text-center mb-16">
                            <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4">
                                Why Choose <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-orange-600">Pathfinder Foundation?</span>
                            </h2>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 lg:gap-10">
                            {[
                                { image: "/WHY PATH IMAGES/Top faculty.webp", label: "Strong Science & Math Base" },
                                { image: "/WHY PATH IMAGES/pathtex app.webp", label: "Pathtex Learning App" },
                                { image: "/WHY PATH IMAGES/Soft skills.webp", label: "Olympiad & NTSE Prep" },
                                { image: "/WHY PATH IMAGES/mental-health.webp", label: "Analytical Thinking" },
                                { image: "/WHY PATH IMAGES/AI.webp", label: "Personalized AI Tracking" },
                                { image: "/WHY PATH IMAGES/Robotics.webp", label: "Robotics & STEM Exposure" }
                            ].map((feature, idx) => (
                                <div
                                    key={idx}
                ...
                {/* Related Foundation Programs Section */}
                <section className="py-20 bg-white relative overflow-hidden">
                    <div className="max-w-6xl mx-auto px-6 relative z-10">
                        <div className="text-center mb-12">
                            <h2 className="text-3xl md:text-5xl font-black text-gray-900 mb-4 tracking-tight">
                                Foundation <span className="text-orange-500">Class 7 to 10 Programs</span>
                            </h2>
                            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
                                Tailored classroom and digital programs nurturing young minds for academic excellence and future competitive exams.
                            </p>
                        </div>
                                    ...
                                    <div key={idx} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-xl hover:shadow-2xl transition-all duration-300 flex flex-col justify-between">
                                        <div>
                                            <span className="inline-block px-3 py-1 bg-orange-100 text-orange-600 font-bold text-xs rounded-full mb-3">
                                                {course.class_level ? `Class ${course.class_level}` : "Class 7 - 10"}
                                            </span>
                                            <h3 className="text-xl font-bold text-gray-900 mb-2">{course.name || course.course_title}</h3>
                                            <p className="text-gray-600 text-sm line-clamp-3 mb-6">{course.short_description || "Comprehensive foundation course building early concept clarity."}</p>
                                        </div>
                                        <button
                                            onClick={() => {
                        ...
                        <div className="bg-white rounded-2xl max-w-sm w-full overflow-hidden shadow-2xl transform animate-in zoom-in-95 duration-300">
                            ...
                            <div className="p-6 text-center space-y-4">
                                <p className="text-sm text-gray-600 leading-relaxed">
                                    Congratulations! We've received your registration for Pathfinder Foundation Program. Our academic counsellors will contact you shortly.
                                </p>
            ...
            <RegistrationPopup 
                ...
                pageSource="Foundation Program"
                showPercentage={popupShowPercentage}
                classOptions={["Class 7", "Class 8", "Class 9", "Class 10"]}
                hideCourseType={true}
            />
        </div>
    );
